...
import {
	ThemedSelect,
	ThemedSelectOption,
} from '@/components/Select/ThemedSelect'
import {
	ModalOverlay,
	ModalContainer,
	ModalHeader,
...
export const JournalEntryModal = ({
	...
	const tagOptions: ThemedSelectOption[] = tags.map(t => ({
		value: t.id,
		label: t.name,
	}))
						...
						<ModalContent $isFullscreen={isFullscreen}>
									...
									<SelectsRow>
										<SelectGroup>
											...
											<ThemedSelect
												...
												value={
													entryTypeOptions.find(o => o.value === type) || null
												}
												onChange={option =>
													setType(
														(option as ThemedSelectOption)
										...
										<SelectGroup>
											...
											<ThemedSelect
												...
												onChange={option =>
													setFolderId(
														(option as ThemedSelectOption)?.value || null,
													)
												}
												isClearable
											/>
								...
								<EntryInfoContainer>
									...
									{entry?.folder && (
										<InfoRow>
											...
											<InfoValue>{entry.folder.name}</InfoValue>
										</InfoRow>
									)}
							...
							{!isViewing && (
								<ButtonGroup $isFullscreen={isFullscreen}>
									...
									<SaveButton
										onClick={handleSave}
										disabled={isLoading || !title.trim() || !content}
									>
										{isLoading
											? isEditing
												? 'Сохранение...'
